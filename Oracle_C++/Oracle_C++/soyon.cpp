...
void displayOrderStatus(Connection* conn, int orderId, int customerId) {
    ...
    if (order == 0) std::cout << "Order ID is not valid." << std::endl;
    else {
        string status{};
        ...
        if (status.compare("null") == 0) std::cout << "Order does not exist" << std::endl;
        else {
...
void cancelOrder(Connection* conn, int orderId, int customerId) {
    ...
    if (order == 0) std::cout << "Order ID is not valid." << endl;
    else {
        int status{};
        ...
        else if (status == 1) std::cout << "The order has been already canceled." << endl;
        else if (status == 2) std::cout << "The order is shipped and cannot be canceled." << endl;
        else if (status == 3) std::cout << "The order is canceled successfully." << endl;
    }
    conn->terminateStatement(stmt);
}
...
int customerLogin(Connection* conn, int customerId) {
    ...
    stmt->executeUpdate();
    customer = stmt->getInt(2);
    if (customer > 0) found = 1;
    conn->terminateStatement(stmt);
